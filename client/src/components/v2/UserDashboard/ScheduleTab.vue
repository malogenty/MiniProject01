...
export default {
  async mounted() {
    ...
    Object.values(this.watchedUser.schedule).forEach((ev) => {
      const calEv = this.$refs.vuecal.createEvent(
        new Date(ev.start),
        ev.duration * 60,
        { title: ev.title }
      )
  ...
  methods: {
    ...
    async editView({ startDate, endDate }) {
      ...
      await this.fetchSchedule({
        u_id: this.givenUser.id,
        from: moment(this.startDate).subtract(1, 'week').format('YYYY-MM-DD'),
        to: moment(this.endDate).add(1, 'week').format('YYYY-MM-DD')
      })
    },
